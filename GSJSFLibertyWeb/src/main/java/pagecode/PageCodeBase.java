...
public abstract class PageCodeBase {
	...
	private static String className = PageCodeBase.class.getName();		
	...
	protected Object resolveExpression(String expression) {
		...
		if ((expression.indexOf("#{") != -1)
			...
			value =
				getFacesContext().getApplication().getExpressionFactory().createValueExpression(expression, Object.class).getValue(getFacesContext().getELContext());
		} else {
			value = expression;
		}
		return value;
	...
	protected void log(String message) {
		...
		logger.logp(Level.FINE, className, methodName, message);
